...
public class ExamController {
    ...
    @Operation(summary = "提交考试答案", description = "学生提交考试答案，系统记录答题情况并转入后台阅卷")
    public Result<Void> submitAnswers(
        ...
        ExamRecord examRecord = examService.submitExam(examRecordId, answers);
        if (ExamRecord.STATUS_PENDING_GRADE.equals(examRecord.getStatus())) {
            kimiGradingService.enqueueGrading(examRecordId);
        }
        return Result.success("答案提交成功，系统正在后台阅卷");
    }

    @PostMapping("/{examRecordId}/grade")
    @Operation(summary = "触发AI阅卷", description = "将试卷加入后台阅卷队列，支持失败后重试")
    public Result<ExamRecord> gradeExam(
            @Parameter(description = "考试记录ID") @PathVariable Integer examRecordId) {
        ExamRecord examRecord = examService.gradeExam(examRecordId);
        if (!ExamRecord.STATUS_GRADED.equals(examRecord.getStatus())
                && !ExamRecord.STATUS_GRADING.equals(examRecord.getStatus())) {
            kimiGradingService.retryGrading(examRecordId);
        }
        return Result.success(examRecord, "已触发后台阅卷");
    }
